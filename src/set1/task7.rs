...
pub fn decrypt_aes_128(key: &[u8], data: &[u8]) -> Vec<u8> {
    ...
    let mut input_buf = RefReadBuffer::new(data);
    let mut output_buf = RefWriteBuffer::new(&mut decrypted);

    let mut decryptor = aes::ecb_decryptor(aes::KeySize::KeySize128, key, NoPadding);
    let _decrypt_result = decryptor
...
mod test {
    ...
    #[test]
    fn aes_ecb_decrypt() {
        let mut file = File::open("assets/task7.txt").expect("Failed to open file");
